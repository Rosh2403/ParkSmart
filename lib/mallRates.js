...
const MALL_RATE_OVERRIDES = [
  {
    key: "jurong_point",
    aliases: ["jurongpoint", "jurongpointshoppingcentre", "jp1", "jp2", "boonlay", "boonlayway"],
    geofence: { lat: 1.3397, lng: 103.7068, radiusM: 450 },
...
export function calculateMallOverrideCost({
  ...
  const match = MALL_RATE_OVERRIDES.find((m) =>
    m.aliases.some((a) => {
      ...
      return alias && (idNorm.includes(alias) || nameNorm.includes(alias) || areaNorm.includes(alias));
    }) || (
      m.geofence &&
