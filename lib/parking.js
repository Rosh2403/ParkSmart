...
import { calculateMallOverrideCost } from "./mallRates.js";

const LTA_API_URL = "https://datamall2.mytransport.sg/ltaodataservice/CarParkAvailabilityv2";

function isLikelyMallDestination(name = "") {
  const normalized = String(name).trim().toLowerCase();
  if (!normalized) return false;
  return /(mall|plaza|square|centre|center|point|city|galleria|atrium|hub)$/i.test(normalized)
    || /(mall|plaza|square|centre|center|point|city|galleria|atrium|hub)\b/i.test(normalized);
}
...
export async function fetchCarparkAvailability(apiKey) {
  ...
  while (hasMore) {
    ...
    if (!data || !Array.isArray(data.value)) {
      throw new Error("Unexpected LTA API response format.");
    }
  ...
  if (allCarparks.length === 0) {
    throw new Error("LTA API returned no carpark records. Please try again shortly.");
  }
...
export function parseLocation(locationStr) {
  ...
  const nums = locationStr.match(/-?\d+(?:\.\d+)?/g);
  if (!nums || nums.length < 2) return null;
  const a = parseFloat(nums[0]);
  const b = parseFloat(nums[1]);
  if (isNaN(a) || isNaN(b) || a === 0 || b === 0) return null;

  // LTA payloads have appeared in both orders over time:
  // - "LAT LNG" (e.g. 1.33 103.74)
  // - "LNG LAT" (e.g. 103.74 1.33)
  // Auto-normalize to { lat, lng } by range checks.
  const aLooksLat = a >= -90 && a <= 90;
  const bLooksLat = b >= -90 && b <= 90;
  const aLooksLng = a >= -180 && a <= 180;
  const bLooksLng = b >= -180 && b <= 180;

  if (aLooksLat && bLooksLng) return { lat: a, lng: b };
  if (bLooksLat && aLooksLng) return { lat: b, lng: a };
  return null;
}
 ...
 * @param {string} [carparkName] - carpark/development name for specific mall overrides
...
export function calculateParkingCost(
  ...
  const mallOverride = calculateMallOverrideCost({
    carparkId,
    carparkName,
    carparkArea,
    carparkLat,
  ...
  if (agency === "LTA") {
    ...
    return {
      ...
      capLabel: `Estimated · $${rate.dayCap}/day cap`,
  ...
  if (agency === "URA" || !isCentral) {
    ...
    return {
      ...
      capLabel: nightHours > 0
        ? `Estimated · $${rate.nightCap} night cap`
        : `Estimated · $${rate.dayCap}/day cap`,
 ...
 */
export function processCarparks(
  rawCarparks,
  destLat,
  destLng,
  durationHours,
  ...
  for (const cp of rawCarparks) {
    ...
    if (isFreeDay && isFpsCarpark) {
      ...
      if (withinFpsWindow) {
        ...
        if (endTime <= cutoverMs) {
          ...
          const paidCostInfo = calculateParkingCost(
            agency,
            paidDurationHours,
            isCentral,
            cutoverMs,
    ...
    const requiresAppPayment = agency !== "LTA";
    const hasGantry = !requiresAppPayment;
  ...
  // If a known mall destination has an official override but LTA did not return
  // a directly identifiable carpark record, inject a virtual mall row so users
  // can still see/test the official mall pricing.
  const destinationOverride = calculateMallOverrideCost({
    carparkId: "",
    carparkName: destinationName,
    carparkArea: "",
    carparkLat: destLat,
    carparkLng: destLng,
    destinationName,
  ...
  const destinationFallback = (!destinationOverride && isLikelyMallDestination(destinationName))
  ...
  if (destinationOverride || destinationFallback) {
    const hasDestinationMallRow = processed.some((cp) =>
      (cp.rateSource === "official" || cp.rateSource === "estimated") &&
      cp.distanceKm <= 0.6 &&
      cp.agency === "LTA"
    );

    if (!hasDestinationMallRow) {
      const mallCostInfo = destinationOverride || destinationFallback;
      const isOfficialDestinationOverride = mallCostInfo?.rateSource === "official";
      const virtualMall = {
        id: `MALL-${String(destinationName || "DEST").toUpperCase().replace(/[^A-Z0-9]+/g, "-")}`,
        name: destinationName
          ? `${destinationName} (${isOfficialDestinationOverride ? "Official Mall Rates" : "Estimated Mall Rates"})`
          : "Mall Parking",
        agency: "LTA",
        area: "",
        lat: destLat,
        ...
        distanceKm: 0,
        walkTimeMin: 0,
        availableLots: -1, // unknown lot count for virtual rows
        isCentral: false,
        isFreeToday: false,
        requiresAppPayment: false,
        hasGantry: true,
        isVirtualMall: true,
        ...mallCostInfo,
      };
      // Pin this row to the top for mall-rate verification flows.
      virtualMall.score = 999;
      processed.push(virtualMall);
    }
  }
